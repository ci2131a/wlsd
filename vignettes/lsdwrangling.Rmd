...
Additional simplifications are also possible. Since all individuals start at the same time pointthe start variable can be ommitted. That is, in this example, since each individual enrolls in the study at time 0, we do not need the start variable column. It can then be clearly stated all individuals start at time 0  and we need only include the possible event time. Additionally, when each row corresponds to only one individual, some data sets may ommit the identification variable. This way, it is further stated that each row corresponds to a unique individual. Table 4 shows one of the simplest data formats that can be used in application with our example.
